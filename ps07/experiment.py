...
pf = ParticleFilter(frame, template, num_particles=300, sigma_mse=10, sigma_dyn=10, template_coords=template_rect)

cv2.imwrite(os.path.join(output_dir, 'out.png'), pf.template)

dist = pf.particles - [cx, cy]
np.sqrt(np.sum(dist**2, axis=1))
np.where(np.sqrt(np.sum(dist**2, axis=1)) < 35)

x, y, w, h = pf._get_box(pf.particles[5], pf.template)
image = frame[y:y+h, x:x+w]
cv2.imwrite(os.path.join(output_dir, 'out1.png'), image)

cx = template_rect['x'] + template_rect['w']/2
cy = template_rect['y'] + template_rect['h']/2
"""
...
def main():
    ...
    # RESULT:
    # low sigma_mse: converge solver/may not converge, because similarity (weight) becomes higher
