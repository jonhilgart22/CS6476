...
def part_1a():
    ...
    dmax = 5  # You may have to try different values
...
def part_2a(get_disp=True):
    ...
    sigma = 0.05  # You may have to try different values
...
def part_2b(get_disp=True):
    ...
    if get_disp:
        w_size = (7, 7)  # You may have to try different values
        dmax = 120  # You may have to try different values
        d_l = disparity_ssd(image_l, image_r, 0, w_size, dmax)
        d_r = disparity_ssd(image_l, image_r, 1, w_size, dmax)
...
def part_3b_1():
    ...
    filter_size = 7
    image_l = cv2.GaussianBlur(image_l, (filter_size, filter_size), sigmaX=0, sigmaY=0)
    image_r = cv2.GaussianBlur(image_r, (filter_size, filter_size), sigmaX=0, sigmaY=0)
